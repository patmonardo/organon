...
export interface Str { kind: 'str'; value: string }
export interface Sym { kind: 'sym'; name: string }
export interface Var { kind: 'var'; name: string }

export type Term = Num | Str | Sym | Var

export interface Atom { pred: string; args: Term[] }
export interface Neg { kind: 'neg'; atom: Atom }
export type BuiltinOp = 'eq' | 'neq' | 'lt' | 'le' | 'gt' | 'ge' | 'add' | 'sub' | 'mul' | 'div'
export interface Builtin { kind: 'builtin'; op: BuiltinOp; args: Term[] }
...
export const bi = (op: BuiltinOp, args: Term[]): Builtin => ({ kind: 'builtin', op, args })

// Match current call sites: agg('count', N, quality(X,Q), undefined, [X])
export const agg = (fun: AggFun, into: Var, over: Atom, value?: Var, by?: Var[]): Aggregate => ({
  kind: 'agg',
  fun,
  over,
  into,
...
export const rule = (head: Atom, body: Literal[], opts?: Partial<Rule>): Rule =>
  ({ head, body, ...(opts ?? {}) })
// New: fact helper (accepts optional metadata like tags)
export const fact = (a: Atom, opts?: Partial<Fact>): Fact => ({ atom: a, ...(opts ?? {}) })
...
function arityOf(a: Atom): number { return a.args.length }
...
export function validate(p: Program): ValidationIssue[] {
  ...
  const noteSig = (pred: string, arity: number, where: string) => {
    ...
    if (prev === undefined) sig.set(pred, arity)
      ...
      issues.push({ kind: 'error', msg: `Arity mismatch for ${pred}: saw ${arity}, expected ${prev}`, where })
    }
  }
  ...
  const varsInAtom = (a: Atom, out: Set<string>) => a.args.forEach(t => varsInTerm(t, out))
  ...
  p.rules?.forEach((r, i) => {
      ...
      switch ((lit as any).kind) {
        case 'builtin': {
          ...
          if (k2.has(b.op) && b.args.length !== 2) {
            issues.push({ kind: 'error', msg: `Builtin ${b.op} expects 2 args, got ${b.args.length}`, where: lwhere })
          }
          if (k3.has(b.op) && b.args.length !== 3) {
            issues.push({ kind: 'error', msg: `Builtin ${b.op} expects 3 args, got ${b.args.length}`, where: lwhere })
          }
        ...
        case 'agg': {
          ...
          if ((a.into as any).kind !== 'var') {
            issues.push({ kind: 'error', msg: `Aggregate.into must be a Var`, where: lwhere })
          } else {
            bodyVars.add((a.into as Var).name)
          }
          a.by?.forEach(v => bodyVars.add(v.name))
          if (a.value && (a.value as any).kind === 'var') bodyVars.add((a.value as Var).name)
          break
        }
        default: {
    ...
    headVars.forEach(vn => {
      if (!bodyVars.has(vn)) {
        issues.push({ kind: 'error', msg: `Head variable ${vn} does not appear in body (range restriction)`, where })
      }
    })
  })
