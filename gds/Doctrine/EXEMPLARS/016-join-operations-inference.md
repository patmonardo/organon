...
**Source file**: `gds/examples/collections_join_operations.rs`
**Arc position**: Inference (Syllogism → Inference across Concept populations)
**Prior exemplar**: [015 — Order/Group Exprs as Syllogism](015-order-group-syllogism.md)
**Next exemplar**: [017 — Streaming Dataset as Process](017-streaming-dataset-process.md)
...
This has immediate doctrinal consequences: when building a GDSL `procedure` that emits
...
In GDSL terms, a semi join corresponds to a `syllogism` block that uses a reference
...
*See*: [GDSL Grammar — query/syllogism](../REFERENCES/gdsl/grammar.md)
...
- When reviewing a GDSL program's `query` block, notice whether it uses filtering that corresponds to inner-join semantics (only confirmed entities) or left-join semantics (all entities of a given population, with corroboration where available). That choice encodes an epistemological stance.
...
- GDSL `query` block syntax for cross-population inference (see [GDSL Grammar](../REFERENCES/gdsl/grammar.md))
