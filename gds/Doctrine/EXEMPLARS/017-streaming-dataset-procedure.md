...
**Source file**: `gds/examples/collections_streaming_dataset.rs`
**Arc position**: Procedure (Inference → Procedure → Process emission)
**Prior exemplar**: [016 — Join Operations as Inference](016-join-operations-inference.md)
**Next exemplar**: [018 — Streaming Lazy as Deferred Collection](018-streaming-lazy-deferred.md)
...
In a fully-specified GDSL program, the Dataset was built by the compiler from a `procedure` block —
...
This is the analog of the GDSL `procedure` block's configuration clauses.
...
*See*: [Core Concepts — Dataset](../REFERENCES/collections-dataset/core-concepts.md)

**`StreamingDataset`** — The Procedure body: a Dataset wrapped in a traversal mechanism with optional transformation.
*See*: [Core Concepts — Pipeline](../REFERENCES/collections-dataset/core-concepts.md)
...
- Study the `with_transform` closure. It is a compact GDSL program written in Rust: derivation → classification → filter → group → aggregate. Every step is an arc moment. The fact that this compiles to a lazy plan and executes in one pass is the ZeroCopy principle at the Procedure level.
...
- GDSL `procedure` block syntax with `emit <stage> dataset` (see [GDSL Grammar](../REFERENCES/gdsl/grammar.md))
- Artifact table materialization and provenance recording (see [Artifact Materialization](../REFERENCES/collections-dataset/artifact-materialization.md))
