...
**Source file**: `gds/examples/collections_string_namespace_rustscript.rs`
**Arc position**: Observation (text-form mark derivation — the NLP entry point)
**Prior exemplar**: [022 — List Namespace as Variable-Arity Observation](022-list-namespace-observation.md)
**Next exemplar**: [024 — Catalog Extension as Artifact Persistence](024-catalog-extension-persistence.md)
...
In the GDSL arc, each row is a separate entity: English-pear is not the same entity as Dutch-peer.
...
*See*: [Unification](../REFERENCES/collections-dataset/unification.md)
...
In a GDSL program, a `concept` block with a string identity field uses this access path:
```gdsl
...
- GDSL `appearance` and `mark` blocks for text columns (see [GDSL Grammar](../REFERENCES/gdsl/grammar.md))
- Multilingual concept unification (see [Unification](../REFERENCES/collections-dataset/unification.md))
