...
**Source file**: `gds/examples/dataframe_order_group.rs`
**Arc position**: Syllogism (Judgment → Syllogism)
**Prior exemplar**: [014 — Expr Pipeline as Judgment](../dataframe/014-expr-pipeline-judgment.md)
**Next exemplar**: [016 — Join Operations as Inference](../dataframe/016-join-operations-inference.md)
...
*See*: [Shell External Program Grammar — syllogism block](../../REFERENCES/shell/grammar.md)
...
- The example opens with a subtle doctrinal hint: "the Dataset can drive into view for downstream inspection." This is the Dataset acting as the Syllogism's semantic controller. The Dataset knows `region` is a grouping key because the external `.gdsl` specification named it as such. The DataFrame is driven by that knowledge.
...
- The external `.gdsl` `syllogism` block syntax (see [Shell External Program Grammar](../../REFERENCES/shell/grammar.md))
- Artifact materialization of Syllogism outputs (see [Artifact Materialization](../../REFERENCES/dataset/artifact-materialization.md))
