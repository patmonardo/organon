...
**Source file**: `gds/examples/dataframe_expr_basic.rs`
**Arc position**: Judgment grammar (reusable predicate units, first-class analytic components)
**Prior exemplar**: [018 — Streaming Lazy as Deferred Collection](../dataset/018-streaming-lazy-deferred.md)
**Next exemplar**: [020 — Framing and Chunking as PureForm Shape](../form/020-framing-chunking-pureform.md)
...
In an external `.gdsl` artifact, a `judgment` block names the inference rule:
...
The external artifact compiler will produce a named Expr for this judgment — exactly `expr!(score > 20.0)` —
...
- Named Judgment blocks in external `.gdsl` artifacts (see [Shell External Program Grammar](../../REFERENCES/shell/grammar.md))
- Expr artifact logging for provenance (see [Provenance](../../REFERENCES/dataset/provenance.md))
