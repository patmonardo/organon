...
**Source file**: `gds/examples/dataset_streaming_lazy.rs`
**Arc position**: Procedure (deferred execution / lazy Process)
**Prior exemplar**: [017 — Streaming Dataset as Procedure](../dataset/017-streaming-dataset-procedure.md)
**Next exemplar**: [019 — Expr as Reusable Judgment Grammar](../dataframe/019-expr-basic-judgment-grammar.md)
...
- A future external artifact compiler will emit `GDSLazyFrame` plans automatically from `procedure` blocks. The handwritten plans in this example are what the compiler will generate.
...
- Provenance recording during lazy collection (see [Artifact Materialization](../../REFERENCES/dataset/artifact-materialization.md))
- The full external `.gdsl` `procedure` block that generates this plan (see [Shell External Program Grammar](../../REFERENCES/shell/grammar.md))
