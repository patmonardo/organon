...
**Source file**: `gds/examples/dataset_io_catalog_graphframe.rs`
**Arc position**: GraphFrame (graph-structured Procedure artifacts entering the Dataset catalog)
**Prior exemplar**: [024 — Catalog Extension as Artifact Persistence](../dataset/024-catalog-extension-persistence.md)
**Next exemplar**: [026 — Semantic Meta Pipeline as End-Stage Concept](../dataset/026-semantic-meta-pipeline.md)
...
| Use case | General artifact persistence | Shell/Dataset artifact output |
...
and its role in the Shell program arc (nodes / edges / graph metadata).
...
In an external `.gdsl` `procedure` block, this corresponds to the `emit` clause replacing prior artifacts
...
*See*: [Artifact Materialization](../../REFERENCES/dataset/artifact-materialization.md)
...
- Notice that `write_graph_store_polars32_to_catalog` takes a `CollectionsIoFormat::Parquet` argument. In a future external `.gdsl` `procedure` block: `emit graph dataset format=parquet;` — the format is declared at the procedure level, not hardcoded in the runtime.
...
- Full provenance chain from graph algorithm output to named catalog artifact (see [Provenance](../../REFERENCES/dataset/provenance.md))
- External `.gdsl` `procedure` syntax for graph emission (see [Shell External Program Grammar](../../REFERENCES/shell/grammar.md))
