...
**Source file**: `gds/examples/form_framing_chunking.rs`
**Arc position**: PureForm / Kernel Shape (the structural substrate beneath Dataset and DataFrame)
**Prior exemplar**: [019 — Expr as Reusable Judgment Grammar](../dataframe/019-expr-basic-judgment-grammar.md)
**Next exemplar**: [021 — Array Namespace as Tensor Surface](../dataset/021-array-namespace-observation.md)
...
When an external `.gdsl` artifact declares `source corpus : parquet("path/to/data.parquet")`,
the kernel loads the file as shaped material — rows and columns with types but no mediated Shell program semantics yet.
...
*See*: [PureForm](../../REFERENCES/form/pureform.md)
...
- The full PureForm triad in the GDS kernel's graph algorithm layer (see [PureForm](../../REFERENCES/form/pureform.md))
