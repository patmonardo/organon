...
This reference defines the namespace contract used by Doctrine exemplars,
runnable examples, persisted fixtures, and generator inputs.
...
- **Fold exemplars** explain the large doctrinal movement, such as
  `Frame:Series::Expr`.
- **Namespace examples** exercise one operational namespace, such as
  `dataframe::frame` or `dataframe::lazy`.
- **Fixtures** persist the artifact evidence emitted by a runnable example.

A fixture can prove a claim, but it is not the claim itself. The Doctrine names
the claim; the example executes it; the fixture preserves its evidence.
...
The public roots are ordered as the kernel fold, not as filesystem accident:

```text
form -> shell -> dataframe -> dataset
```

Form defines the PureForm return. Shell is the runtime connector. DataFrame is
the immediate register body. Dataset is the mediated semantic controller.

| Root | Role | Example Prefix | Fixture Prefix |
|---|---|---|---|
| `form` | PureForm, ProgramFeature, principle-gated return | `form_*` | `fixtures/collections/form/<example-stem>` |
| `shell` | unified protocol over immediate and mediated registers | `shell_*` | `fixtures/collections/shell/<example-stem>` |
| `dataframe` | executable Frame:Series::Expr body | `dataframe_*` | `fixtures/collections/dataframe/<example-stem>` |
| `dataset` | mediated Dataset grammar and artifact production | `dataset_*` | `fixtures/collections/dataset/<example-stem>` |

These roots must match across Doctrine exemplars, references, runnable examples,
and fixtures. Do not create a combined DataFrame/Dataset authority. DataFrame
and Dataset touch constantly, but they name different registers.
...
|---|---|---|---|
| `dataframe::frame` | whole tabular enclosure | `collections::dataframe::frame` / `GDSDataFrame` | `dataframe_frame_surface` |
