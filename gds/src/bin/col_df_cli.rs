use gds::collections::dataframe::workbench::dataframe_workbench_track;
use gds::collections::dataframe::workbench::dataframe_workbench_tracks;
use gds::collections::dataframe::workbench::DataFrameWorkbenchTrack;
...
fn run_interactive_menu() {
    loop {
        println!("DataFrame Collections Workbench");
        println!();
        print_tracks();
        println!();
        println!(
            "Select track number/id, batch (1,3-5), or ids (df-a,df-b). Commands: l=list, q=quit"
        );
        print!("> ");
...
fn run_selection_expr(selection: &str) {
    let tracks = dataframe_workbench_tracks();
...
fn run_track_by_id(id: &str) {
    match dataframe_workbench_track(id) {
...
fn run_track(track: &DataFrameWorkbenchTrack) {
...
fn print_help() {
    println!("Collections DataFrame Workbench CLI");
    println!();
    println!("Usage:");
    println!("  cargo run -p gds --bin col_df_cli -- [menu|list|run <selection>]");
    println!();
