//! PGQL Function registry (mirror of polars-sql `function_registry.rs` API)

use polars_error::{polars_bail, PolarsResult};
use polars_plan::prelude::udf::UserDefinedFunction;
pub use polars_plan::prelude::FunctionOptions;

/// A registry that holds user defined functions for PGQL.
...
mod tests {
    ...
    #[test]
    fn default_registry_behaviour() {
        let reg = DefaultFunctionRegistry {};
        assert!(!reg.contains("foo"));
        let f = reg.get_udf("foo");
        assert!(f.is_err());
    }
}
