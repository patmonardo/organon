...
macro_rules! fspec_tree {
    ( $property:expr, [ $( $idx:expr ),+ $(,)? ] ) => {
        $crate::collections::dataset::feature::FeatureSpec::new_tree(
            $property,
            $crate::collections::dataset::dsl::expressions::tree::TreePos::new(vec![
                $( $idx as usize ),+
            ]),
        )
    };
...
macro_rules! fexpr {
    ($expr:expr) => {
        $crate::collections::dataset::dsl::expressions::feature::FeatureExpr::from($expr)
    };
}
...
macro_rules! pipeline {
    () => {
        $crate::collections::dataset::dsl::functions::pipeline()
    };
    (text $base:expr $(,)?) => {
        $crate::collections::dataset::dsl::functions::text_lifecycle($base)
    };
    (ops [ $( $op:expr ),* $(,)? ] $(,)?) => {{
        let mut __pipeline = $crate::collections::dataset::dsl::functions::pipeline();
