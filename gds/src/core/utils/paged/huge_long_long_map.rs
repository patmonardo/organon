...
impl HugeLongLongMap {
    ...
    pub fn memory_estimation(expected_elements: usize) -> usize {
        ...
        Estimate::size_of_long_array(array_size) * 2 + std::mem::size_of::<Self>()
    }
...
mod tests {
    ...
    fn memory_estimation_accounts_for_keys_and_values_and_struct() {
        let estimated = HugeLongLongMap::memory_estimation(100);
        // Should be greater than just the arrays due to struct overhead
        let array_size = HugeLongLongMap::min_buffer_size(100);
