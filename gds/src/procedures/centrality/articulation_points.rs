...
mod tests {
    ...
    fn facade_treats_directed_projection_as_undirected_connectivity() {
        let mut outgoing: Vec<Vec<MappedNodeId>> = vec![Vec::new(); 5];
        let mut incoming: Vec<Vec<MappedNodeId>> = vec![Vec::new(); 5];
        ...
        let store = DefaultGraphStore::new(
            ...
            schema,
            Capabilities::default(),
            SimpleIdMap::from_original_ids((0..5).map(OriginalNodeId::new)),
            relationship_topologies,
        );
        let graph = GraphFacade::new(Arc::new(store));
