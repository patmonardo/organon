...
pub trait NodeClassificationPredictPipelineConfig {
...
#[derive(Debug, Clone)]
pub struct NodeClassificationPredictPipelineBaseConfig {
    graph_name: String,
    concurrency: usize,
    model_name: String,
    model_user: String,
...
impl NodeClassificationPredictPipelineBaseConfig {
    pub fn new(
        graph_name: String,
        concurrency: usize,
        model_name: String,
    ...
    pub fn from_map(username: String, mut config: AnyMap) -> Self {
        ...
        let include_predicted_probabilities = config
            .remove("includePredictedProbabilities")
    ...
    pub fn to_map(&self) -> AnyMap {
        ...
        map.insert(
            "includePredictedProbabilities".to_string(),
            Value::Bool(self.include_predicted_probabilities),
        );
        map
    }
}

impl NodeClassificationPredictPipelineConfig for NodeClassificationPredictPipelineBaseConfig {
    fn graph_name(&self) -> &str {
        &self.graph_name
    }
    ...
    fn include_predicted_probabilities(&self) -> bool {
        self.include_predicted_probabilities
    }
}
...
mod tests {
    ...
    fn test_from_map_leaves_absent_filters_empty() {
        let config = NodeClassificationPredictPipelineBaseConfig::from_map(
            "alice".to_string(),
            AnyMap::new(),
        );
    ...
    fn test_train_config_defaults_preserve_user_filters() {
        ...
        let config =
            NodeClassificationPredictPipelineBaseConfig::from_map("alice".to_string(), map)
                .with_train_config_defaults(&TestTrainConfig);
