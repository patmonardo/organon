...
Status: active
Applies to: all Essence workbooks in this folder
...
## Path-forward principle (LogoLogia)

- Naming principle: `LogoLogia` is the guiding term for this workbook program.
- Program direction: KG-first development, with TopicMap treated only as a fundamental tool (not the governing model).
- Human-in-the-loop requirement: generation is collaborative and line-accountable; reviewer intent governs acceptance.
...
- Current sequencing default for this track: `Shine -> Reflection -> relation-expansion cycles`.
