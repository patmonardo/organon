...
import { attributeIR } from '@relative/essence/actuality/absolute/attribute-ir';
import { expositionIR } from '@relative/essence/actuality/absolute/exposition-ir';
import { modeIR } from '@relative/essence/actuality/absolute/mode-ir';
import { ATTRIBUTE_TOPIC_MAP } from '@relative/essence/actuality/absolute/sources/attribute-topic-map';
import { EXPOSITION_TOPIC_MAP } from '@relative/essence/actuality/absolute/sources/exposition-topic-map';
import { MODE_TOPIC_MAP } from '@relative/essence/actuality/absolute/sources/mode-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'exposition-ir.ts',
    exportName: 'expositionIR',
    statesExportName: 'expositionStates',
    ir: expositionIR,
  },
  {
    fileName: 'attribute-ir.ts',
    exportName: 'attributeIR',
    statesExportName: 'attributeStates',
    ir: attributeIR,
  },
  {
    fileName: 'mode-ir.ts',
    exportName: 'modeIR',
    statesExportName: 'modeStates',
    ir: modeIR,
  },
];
...
async function main() {
  ...
  const boundaryReasons = [
    'Exposition of the absolute passes into the absolute attribute.',
    'Attribute passes into mode as externalized determinateness.',
  ];
  ...
  const topLevelTs = renderTopicMapFile(
    'topicmapActualityAbsoluteTopicMapIR',
    'ActualityAbsoluteTopicMapIR',
    'topicmapActualityAbsoluteTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Actuality-absolute section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
