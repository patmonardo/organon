...
import { essenceIR } from '@relative/essence/reflection/essence/essence-ir';
import { reflectionIR } from '@relative/essence/reflection/essence/reflection-ir';
import { shineIR } from '@relative/essence/reflection/essence/shine-ir';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'essence-ir.ts',
    exportName: 'essenceIR',
    statesExportName: 'essenceStates',
    ir: essenceIR,
  },
  {
    fileName: 'shine-ir.ts',
    exportName: 'shineIR',
    statesExportName: 'shineStates',
    ir: shineIR,
  },
  {
    fileName: 'reflection-ir.ts',
    exportName: 'reflectionIR',
    statesExportName: 'reflectionStates',
    ir: reflectionIR,
  },
];
...
function deriveTags(chunk: TopicMapChunk): string[] {
  ...
  if (body.includes('positing') || body.includes('presupposing')) {
    tags.add('positing');
  }
...
async function main() {
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'Essential/unessential opposition passes into shine as non-essence.',
  });
  ...
  traces.push({
    ...
    type: 'REFLECTS',
    reason:
      'Essence as shining-within-itself opens the explicit reflection chapter.',
  });
  ...
  const topLevelTs = renderTopicMapFile(
    'topicmapReflectionEssenceTopicMapIR',
    'ReflectionEssenceTopicMapIR',
    'topicmapReflectionEssenceTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Reflection-essence section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
