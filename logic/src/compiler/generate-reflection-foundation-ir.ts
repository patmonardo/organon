...
import { contradictionIR } from '@relative/essence/reflection/foundation/contradiction-ir';
import { differenceIR } from '@relative/essence/reflection/foundation/difference-ir';
import { identityIR } from '@relative/essence/reflection/foundation/identity-ir';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'identity-ir.ts',
    exportName: 'identityIR',
    statesExportName: 'identityStates',
    ir: identityIR,
  },
  {
    fileName: 'difference-ir.ts',
    exportName: 'differenceIR',
    statesExportName: 'differenceStates',
    ir: differenceIR,
  },
  {
    fileName: 'contradiction-ir.ts',
    exportName: 'contradictionIR',
    statesExportName: 'contradictionStates',
    ir: contradictionIR,
  },
];
...
function inferTraceType(nextChunk: TopicMapChunk): TraceType {
  ...
  if (body.includes('reflect') || body.includes('foundation'))
    return 'REFLECTS';
  return 'NEXT';
}
...
function deriveTags(chunk: TopicMapChunk): string[] {
  ...
  if (body.includes('difference')) {
    tags.add('difference');
  }
  if (body.includes('opposition') || body.includes('contradiction')) {
    tags.add('contradiction');
  }
  if (body.includes('ground') || body.includes('foundation')) {
    tags.add('ground');
  }
...
async function main() {
  ...
  const topLevelTs = renderTopicMapFile(
    'topicmapReflectionFoundationTopicMapIR',
    'ReflectionFoundationTopicMapIR',
    'topicmapReflectionFoundationTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Reflection-foundation section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
