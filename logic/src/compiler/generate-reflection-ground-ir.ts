...
import { absoluteIR } from '@relative/essence/reflection/ground/absolute-ir';
import { conditionIR } from '@relative/essence/reflection/ground/condition-ir';
import { determinateIR } from '@relative/essence/reflection/ground/determinate-ir';
import { absoluteTopicMap } from '@relative/essence/reflection/ground/sources/absolute-topic-map';
import { conditionTopicMap } from '@relative/essence/reflection/ground/sources/condition-topic-map';
import { determinateTopicMap } from '@relative/essence/reflection/ground/sources/determinate-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'absolute-ir.ts',
    exportName: 'absoluteIR',
    statesExportName: 'absoluteStates',
    ir: absoluteIR,
  },
  {
    fileName: 'determinate-ir.ts',
    exportName: 'determinateIR',
    statesExportName: 'determinateStates',
    ir: determinateIR,
  },
  {
    fileName: 'condition-ir.ts',
    exportName: 'conditionIR',
    statesExportName: 'conditionStates',
    ir: conditionIR,
  },
];
...
function inferTraceType(nextChunk: TopicMapChunk): TraceType {
  ...
  if (body.includes('reflect') || body.includes('ground')) return 'REFLECTS';
  return 'NEXT';
}
...
function deriveTags(chunk: TopicMapChunk): string[] {
  ...
  if (body.includes('matter')) {
    tags.add('matter');
  }
  if (body.includes('content')) {
    tags.add('content');
  }
  if (body.includes('condition')) {
    tags.add('condition');
  }
...
async function main() {
  ...
  const topLevelTs = renderTopicMapFile(
    'topicmapReflectionGroundTopicMapIR',
    'ReflectionGroundTopicMapIR',
    'topicmapReflectionGroundTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Reflection-ground section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
