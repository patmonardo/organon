...
export class AgentEngine extends BaseEngine {
  ...
  constructor(agentId: string, empowerments: EmpowermentLike[] = [], opts?: { scope?: string }) {
  ...
  protected async verifySignatures(sigs?: any[]): Promise<boolean> {
    ...
    return sigs.every((s) => typeof s.signature === 'string' && s.signature.length > 0 && (s.issuer === this.agentId || (s.issuer ?? '').startsWith('agent')));
  }
  ...
  checkFacets(facets: Array<any>, ctx: { action?: string; resource?: string } = {}) {
    ...
    for (const f of facets) {
      if (typeof f === 'string') {
        if (f === '*' || f === action || f === resource || f === this.agentId) return true;
        continue;
      }
