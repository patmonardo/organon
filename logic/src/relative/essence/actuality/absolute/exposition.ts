...
/**
 * A. THE EXPOSITION OF THE ABSOLUTE — Species 1 of The Absolute
 *
 * This module covers the first species: the exposition of the absolute,
 * absolute form and content, negative and positive exposition.
 */
...
export const CANONICAL_CHUNKS: Chunk[] = [
  {
    ...
    text: `The absolute is not just being, nor even essence.
...
and whose ground is not yet posited.`,
    summary: 'Absolute not = being (first unreflected immediacy) nor essence (reflected immediacy). Each = determinate totality. Being emerges in essence = concrete existence. Connection develops = relation of inner and outer. Inner = essence (referred to being, being immediately). Outer = being (connected with reflection, relationless identity with essence). Absolute = absolute unity of two, ground of essential relation.'
  },
  {
    id: 'abs-a-2-absolute-form-and-content',
    title: 'Absolute form and content — each moment totality',
    text: `It follows that the determination of
...
as into its ground.`,
    summary: 'Movement = sublating its act in absolute. Beyond of manifold differences/determinations/movement, beyond at back of absolute = negative exposition. True presentation = preceding whole of logical movement (being and essence). Content not gathered from outside, not sunk by external reflection, but determined itself by inner necessity. Being\'s own becoming and reflection of essence returned into absolute as ground.'
  },
  {
    id: 'abs-a-6-positive-exposition',
    title: 'Positive exposition — finite as expression of absolute',
    text: `But this exposition has itself also a positive side,
...
and determines it as attribute.`,
    summary: 'Absolute not attribute just because subject matter of external reflection. Reflection not only external but immediately internal (because external). Absolute absolute only because not abstract identity but identity of being and essence (inner and outer). Absolute = absolute form that makes it shine within itself and determines as attribute.'
  },
];

export const LOGICAL_OPERATIONS: LogicalOperation[] = [
  {
    id: 'abs-a-op-1-absolute-not-being-nor-essence',
    chunkId: 'abs-a-1-absolute-not-being-nor-essence',
    label: 'Absolute not being nor essence — absolute unity',
    clauses: [
      'absolute.not = being',
      'absolute.not = essence',
      'being = firstUnreflectedImmediacy',
      'essence = reflectedImmediacy',
      'each = determinateTotality',
      'being.emergesInEssence = concreteExistence',
      'connection.developsInto = relationOfInnerAndOuter',
      'inner = essence',
      'inner.referredTo = being',
      'inner = beingImmediately',
      'outer = being',
      'outer.connectedWith = reflection',
      'outer = relationlessIdentityWithEssence',
      'absolute = absoluteUnityOfTwo',
      'absolute = groundOfEssentialRelation',
      'essentialRelation.hasYetToReturn = intoIdentity',
      'ground.notYetPosited = true',
    ],
    predicates: [
      { name: 'IsAbsoluteUnity', args: ['absolute'] },
    ],
    relations: [
      { predicate: 'is', from: 'absolute', to: 'absoluteUnity' },
    ],
  },
  {
    id: 'abs-a-op-2-absolute-form-and-content',
    chunkId: 'abs-a-2-absolute-form-and-content',
    label: 'Absolute form and content — each moment totality',
    clauses: [
      'determinationOfAbsolute = absoluteForm',
      'notIdentityWith = simpleDeterminacies',
      'identity.moments = eachExplicitlyTotality',
      'moments.indifferentTo = form',
      'moments = completeContent',
      'absolute = absoluteContent',
      'content = indifferentPlurality',
      'content.has = negativeConnectionOfForm',
      'manifold = oneSubstantialIdentity',
    ],
    predicates: [
      { name: 'IsAbsoluteForm', args: ['determination'] },
      { name: 'IsAbsoluteContent', args: ['absolute'] },
    ],
    relations: [
      { predicate: 'is', from: 'determination', to: 'absoluteForm' },
    ],
  },
  {
    id: 'abs-a-op-3-absolute-identity-transparent',
    chunkId: 'abs-a-3-absolute-identity-transparent',
    label: 'Absolute identity — transparent reflective shine',
    clauses: [
      'identityOfAbsolute = absoluteIdentity',
      'eachPart = whole',
      'eachDeterminateness = totality',
      'determinateness = transparentReflectiveShine',
      'difference.disappearedIn = positedness',
      'reflectedDeterminations.appearAs = trueBeing',
      'absolute = ground',
      'determinations.founderedInto = absolute',
    ],
    predicates: [
      { name: 'IsTransparentReflectiveShine', args: ['determinateness'] },
    ],
    relations: [
      { predicate: 'is', from: 'determinateness', to: 'transparentReflectiveShine' },
    ],
  },
  {
    id: 'abs-a-op-4-absolute-does-not-determine',
    chunkId: 'abs-a-4-absolute-does-not-determine',
    label: 'Absolute does not determine itself — movement stands over',
    clauses: [
      'inAbsolute.form = simpleSelfIdentity',
      'absolute.doesNotDetermineItself = true',
      'determination = differenceOfForm',
      'absolute.contains = everyDifference',
      'absolute.contains = formDetermination',
      'absolute = absoluteFormAndReflection',
      'differenceOfContent.mustComeInto = absolute',
      'absolute = absoluteIdentity',
      'manifoldness.sublatedIn = absolute',
      'inAbsolute.noBecoming = true',
      'absolute.not = being',
      'absolute.notDeterminesReflectively = true',
      'absolute.not = essence',
      'absolute.notExternalizes = true',
      'absolute = identityOfInnerAndOuter',
      'movementOfReflection.standsOverAgainst = absoluteIdentity',
      'movement.sublatedIn = identity',
      'movement = inner',
      'movement.consequently = outer',
    ],
    predicates: [
      { name: 'DoesNotDetermineItself', args: ['absolute'] },
    ],
    relations: [
      { predicate: 'standsOverAgainst', from: 'movementOfReflection', to: 'absoluteIdentity' },
    ],
  },
  {
    id: 'abs-a-op-5-negative-exposition',
    chunkId: 'abs-a-5-negative-exposition',
    label: 'Negative exposition — beyond of differences',
    clauses: [
      'movement.consistsIn = sublatingActInAbsolute',
      'movement = beyondOfDifferences',
      'beyond.liesAtBackOf = absolute',
      'beyond = negativeExposition',
      'exposition = precedingWholeOfLogicalMovement',
      'logicalMovement.of = {being, essence}',
      'content.notGatheredFrom = outside',
      'content.notSunkBy = externalReflection',
      'content.determinedItselfBy = innerNecessity',
      'content.as = {beingsOwnBecoming, reflectionOfEssence}',
      'content.returnedInto = absolute',
      'absolute.as = ground',
    ],
    predicates: [
      { name: 'IsNegativeExposition', args: ['exposition'] },
    ],
    relations: [
      { predicate: 'is', from: 'exposition', to: 'negativeExposition' },
    ],
  },
  {
    id: 'abs-a-op-6-positive-exposition',
    chunkId: 'abs-a-6-positive-exposition',
    label: 'Positive exposition — finite as expression of absolute',
    clauses: [
      'exposition.has = positiveSide',
      'finite.founderingDemonstrates = nature',
      'nature = referredToAbsolute',
      'nature = containsAbsolute',
      'positiveSide = expositionOfDeterminations',
      'determinations.haveAbsoluteFor = {abyss, ground}',
      'absolute.impartsSubsistenceTo = reflectiveShine',
      'beingAsShine = reflection',
      'beingAsShine = referenceToAbsolute',
      'shine.shines = absolute',
      'positiveExposition.halts = finite',
      'finite.consideredAs = {expression, copy}',
      'transparency.lets = absoluteTranspire',
      'transparency.endsIn = completeDisappearance',
      'finite.hasNothingRetaining = difference',
      'finite.absorbedBy = thatThroughWhichItShines',
    ],
    predicates: [
      { name: 'IsPositiveExposition', args: ['exposition'] },
    ],
    relations: [
      { predicate: 'is', from: 'exposition', to: 'positiveExposition' },
    ],
  },
  {
    id: 'abs-a-op-7-positive-exposition-reflective-shine',
    chunkId: 'abs-a-7-positive-exposition-reflective-shine',
    label: 'Positive exposition as reflective shine — absolute\'s own doing',
    clauses: [
      'positiveExposition = reflectiveShine',
      'truePositive = absoluteItself',
      'absolute.contains = {exposition, expoundedContent}',
      'formInWhichAbsoluteShines = nullity',
      'nullity.gatheredFrom = outside',
      'nullity = startingPoint',
      'determination.hasInAbsolute = end',
      'determination.notHasInAbsolute = beginning',
      'expositoryProcess = absoluteAct',
      'expositoryProcess.returnsTo = absolute',
      'startingPoint = externalDetermination',
      'butInFact.exposition = absolutesOwnDoing',
      'act.beginsFrom = itself',
      'act.arrivesAt = itself',
      'absoluteAsAbsoluteIdentity = determinedGuise',
      'absoluteAsAbsoluteIdentity = identicalAbsolute',
      'positedByReflection.overAgainst = {opposition, manifoldness}',
      'absolute = negativeOfReflection',
      'absolute = negativeOfDetermination',
      'exposition.incomplete = true',
      'absoluteItself.incomplete = true',
      'absoluteAsAbsoluteIdentity = absoluteOfExternalReflection',
      'absolute.not = absolutelyAbsolute',
      'absolute = absoluteInDetermination',
      'absolute = attribute',
    ],
    predicates: [
      { name: 'IsAbsolutesOwnDoing', args: ['exposition'] },
    ],
    relations: [
      { predicate: 'is', from: 'exposition', to: 'absolutesOwnDoing' },
    ],
  },
  {
    id: 'abs-a-op-8-absolute-as-attribute',
    chunkId: 'abs-a-8-absolute-as-attribute',
    label: 'Absolute as attribute — absolute form',
    clauses: [
      'absolute.notAttributeJustBecause = subjectMatterOfExternalReflection',
      'reflection.notOnly = external',
      'reflection.becauseExternal = immediatelyInternal',
      'absolute.absoluteOnlyBecause = notAbstractIdentity',
      'absolute = identityOfBeingAndEssence',
      'absolute = identityOfInnerAndOuter',
      'absolute = absoluteForm',
      'absoluteForm.makesShine = withinItself',
      'absoluteForm.determinesAs = attribute',
    ],
    predicates: [
      { name: 'IsAbsoluteForm', args: ['absolute'] },
    ],
    relations: [
      { predicate: 'is', from: 'absolute', to: 'absoluteForm' },
    ],
  },
];
