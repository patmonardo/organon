...
import { absoluteNecessityIR } from '@relative/essence/actuality/actuality/absolute-necessity-ir';
import { contingencyIR } from '@relative/essence/actuality/actuality/contingency-ir';
import { relativeNecessityIR } from '@relative/essence/actuality/actuality/relative-necessity-ir';
import { ABSOLUTE_NECESSITY_TOPIC_MAP } from '@relative/essence/actuality/actuality/sources/absolute-necessity-topic-map';
import { CONTINGENCY_TOPIC_MAP } from '@relative/essence/actuality/actuality/sources/contingency-topic-map';
import { RELATIVE_NECESSITY_TOPIC_MAP } from '@relative/essence/actuality/actuality/sources/relative-necessity-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'contingency-ir.ts',
    exportName: 'contingencyIR',
    statesExportName: 'contingencyStates',
    ir: contingencyIR,
  },
  {
    fileName: 'relative-necessity-ir.ts',
    exportName: 'relativeNecessityIR',
    statesExportName: 'relativeNecessityStates',
    ir: relativeNecessityIR,
  },
  {
    fileName: 'absolute-necessity-ir.ts',
    exportName: 'absoluteNecessityIR',
    statesExportName: 'absoluteNecessityStates',
    ir: absoluteNecessityIR,
  },
];
...
function inferTraceType(nextChunk: IntegratedChunk): TraceType {
  ...
  if (body.includes('reflect') || body.includes('necessity')) return 'REFLECTS';
  return 'NEXT';
}
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('actuality')) {
    tags.add('actuality');
  }
  if (body.includes('possibility')) {
    tags.add('possibility');
  }
  if (body.includes('necessity')) {
    tags.add('necessity');
  }
  if (body.includes('contingen')) {
    tags.add('contingency');
  }
...
async function main() {
  ...
  const sourceSpecs: SourceSpec[] = [
    {
      ...
      title: 'A. CONTINGENCY',
      sourceFile:
        'relative/essence/actuality/actuality/sources/contingency.txt',
      topicMap: CONTINGENCY_TOPIC_MAP.entries,
    },
    {
      id: 'source-relative-necessity',
      title: 'B. RELATIVE NECESSITY',
      sourceFile:
        'relative/essence/actuality/actuality/sources/relative-necessity.txt',
      topicMap: RELATIVE_NECESSITY_TOPIC_MAP.entries,
    },
    {
      id: 'source-absolute-necessity',
      title: 'C. ABSOLUTE NECESSITY',
      sourceFile:
        'relative/essence/actuality/actuality/sources/absolute-necessity.txt',
      topicMap: ABSOLUTE_NECESSITY_TOPIC_MAP.entries,
    },
  ];
  ...
  const boundaryReasons = [
    'Formal contingency/necessity transitions into relative necessity with content.',
    'Relative necessity transitions into absolute necessity as self-grounding truth.',
  ];
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedActualityActualityTopicMapIR',
    'IntegratedActualityActualityTopicMapIR',
    'integratedActualityActualityTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Actuality section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
