...
import { dissolutionIR } from '@relative/essence/appearance/thing/dissolution-ir';
import { matterIR } from '@relative/essence/appearance/thing/matter-ir';
import { thingIR } from '@relative/essence/appearance/thing/thing-ir';
import { dissolutionTopicMap } from '@relative/essence/appearance/thing/sources/dissolution-topic-map';
import { matterTopicMap } from '@relative/essence/appearance/thing/sources/matter-topic-map';
import { thingTopicMap } from '@relative/essence/appearance/thing/sources/thing-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'thing-ir.ts',
    exportName: 'thingIR',
    statesExportName: 'thingStates',
    ir: thingIR,
  },
  {
    fileName: 'matter-ir.ts',
    exportName: 'matterIR',
    statesExportName: 'matterStates',
    ir: matterIR,
  },
  {
    fileName: 'dissolution-ir.ts',
    exportName: 'dissolutionIR',
    statesExportName: 'dissolutionStates',
    ir: dissolutionIR,
  },
];
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-appearance-thing-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'Thinghood passes into constitution through self-subsistent matters.',
  });
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'Constitution out of matters culminates in dissolution of the thing.',
  });
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedAppearanceThingTopicMapIR',
    'IntegratedAppearanceThingTopicMapIR',
    'integratedAppearanceThingTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Appearance-thing section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
