...
import { disappearanceIR } from '@relative/essence/appearance/world/disappearance-ir';
import { lawIR } from '@relative/essence/appearance/world/law-ir';
import { worldIR } from '@relative/essence/appearance/world/world-ir';
import { disappearanceTopicMap } from '@relative/essence/appearance/world/sources/disappearance-topic-map';
import { lawTopicMap } from '@relative/essence/appearance/world/sources/law-topic-map';
import { worldTopicMap } from '@relative/essence/appearance/world/sources/world-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'world-ir.ts',
    exportName: 'worldIR',
    statesExportName: 'worldStates',
    ir: worldIR,
  },
  {
    fileName: 'law-ir.ts',
    exportName: 'lawIR',
    statesExportName: 'lawStates',
    ir: lawIR,
  },
  {
    fileName: 'disappearance-ir.ts',
    exportName: 'disappearanceIR',
    statesExportName: 'disappearanceStates',
    ir: disappearanceIR,
  },
];
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('world')) {
    tags.add('world');
  }
  if (body.includes('law')) {
    tags.add('law');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-appearance-world-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  const sourceSpecs: SourceSpec[] = [
    ...
    },
    {
      ...
      topicMap: disappearanceTopicMap,
    },
  ];
  ...
  traces.push({
    ...
    type: 'REFLECTS',
    reason:
      'Inversion of the world of appearance transitions into the law of appearance.',
  });
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'The law as not-yet-real form passes into disappearance through opposed worlds.',
  });
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedAppearanceWorldTopicMapIR',
    'IntegratedAppearanceWorldTopicMapIR',
    'integratedAppearanceWorldTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Appearance-world section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
