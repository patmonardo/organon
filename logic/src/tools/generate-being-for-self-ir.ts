...
import { attractionTopicMap } from '@relative/being/quality/being-for-self/sources/attraction-topic-map';
import { beingForSelfTopicMap } from '@relative/being/quality/being-for-self/sources/being-for-self-topic-map';
import { oneManyTopicMap } from '@relative/being/quality/being-for-self/sources/one-many-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'being-for-self-ir.ts',
    exportName: 'beingForSelfIR',
    statesExportName: 'beingForSelfStates',
    ir: beingForSelfIR,
  },
  {
    fileName: 'one-many-ir.ts',
    exportName: 'oneManyIR',
    statesExportName: 'oneManyStates',
    ir: oneManyIR,
  },
  {
    fileName: 'attraction-ir.ts',
    exportName: 'attractionIR',
    statesExportName: 'attractionStates',
    ir: attractionIR,
  },
];
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('repulsion')) {
    tags.add('repulsion');
  }
  if (body.includes('attraction')) {
    tags.add('attraction');
  }
  if (body.includes('infinity')) {
    tags.add('infinity');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-being-for-self-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  const sourceSpecs: SourceSpec[] = [
    {
      ...
      sourceFile:
        'relative/being/quality/being-for-self/sources/being-for-self.txt',
      topicMap: beingForSelfTopicMap,
    },
    {
      id: 'source-one-many',
      title: 'The one and the many',
      sourceFile: 'relative/being/quality/being-for-self/sources/one-many.txt',
      topicMap: oneManyTopicMap,
    },
    {
      id: 'source-attraction',
      title: 'Repulsion and attraction',
      sourceFile:
        'relative/being/quality/being-for-self/sources/attraction.txt',
      topicMap: attractionTopicMap,
    },
  ];
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'Transition from repulsion in plurality to repulsion and attraction.',
  });
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedBeingForSelfTopicMapIR',
    'IntegratedBeingForSelfTopicMapIR',
    'integratedBeingForSelfTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Being-for-self section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
