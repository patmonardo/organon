...
import { limitingQuantityIR } from '@relative/being/quantity/quantity/limiting-quantity-ir';
import { magnitudeIR } from '@relative/being/quantity/quantity/magnitude-ir';
import { pureQuantityIR } from '@relative/being/quantity/quantity/pure-quantity-ir';
import { LIMITING_QUANTITY_TOPIC_MAP } from '@relative/being/quantity/quantity/sources/limiting-quantity-topic-map';
import { CONTINUOUS_DISCRETE_MAGNITUDE_TOPIC_MAP } from '@relative/being/quantity/quantity/sources/magnitude-topic-map';
import { PURE_QUANTITY_TOPIC_MAP } from '@relative/being/quantity/quantity/sources/pure-quantity-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'pure-quantity-ir.ts',
    exportName: 'pureQuantityIR',
    statesExportName: 'pureQuantityStates',
    ir: pureQuantityIR,
  },
  {
    fileName: 'magnitude-ir.ts',
    exportName: 'magnitudeIR',
    statesExportName: 'magnitudeStates',
    ir: magnitudeIR,
  },
  {
    fileName: 'limiting-quantity-ir.ts',
    exportName: 'limitingQuantityIR',
    statesExportName: 'limitingQuantityStates',
    ir: limitingQuantityIR,
  },
];
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('continuous')) {
    tags.add('continuous');
  }
  if (body.includes('discrete')) {
    tags.add('discrete');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-quantity-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  const sourceSpecs: SourceSpec[] = [
    {
      ...
      topicMap: PURE_QUANTITY_TOPIC_MAP.entries,
    },
    {
      id: 'source-magnitude',
      title: 'B. CONTINUOUS AND DISCRETE MAGNITUDE',
      sourceFile: 'relative/being/quantity/quantity/sources/magnitude.txt',
      topicMap: CONTINUOUS_DISCRETE_MAGNITUDE_TOPIC_MAP.entries,
    },
    {
      id: 'source-limiting-quantity',
      title: 'C. THE LIMITING OF QUANTITY',
      sourceFile:
        'relative/being/quantity/quantity/sources/limiting-quantity.txt',
      topicMap: LIMITING_QUANTITY_TOPIC_MAP.entries,
    },
  ];
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'Pure quantity as whole transitions into explicit magnitude duality.',
  });
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedQuantityTopicMapIR',
    'IntegratedQuantityTopicMapIR',
    'integratedQuantityTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Quantity section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
