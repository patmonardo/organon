...
import { infinityIR } from '@relative/being/quantity/quantum/infinity-ir';
import { numberIR } from '@relative/being/quantity/quantum/number-ir';
import { quantumIR } from '@relative/being/quantity/quantum/quantum-ir';
import { INFINITY_TOPIC_MAP } from '@relative/being/quantity/quantum/sources/infinity-topic-map';
import { NUMBER_TOPIC_MAP } from '@relative/being/quantity/quantum/sources/number-topic-map';
import { QUANTUM_TOPIC_MAP } from '@relative/being/quantity/quantum/sources/quantum-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'number-ir.ts',
    exportName: 'numberIR',
    statesExportName: 'numberStates',
    ir: numberIR,
  },
  {
    fileName: 'quantum-ir.ts',
    exportName: 'quantumIR',
    statesExportName: 'quantumStates',
    ir: quantumIR,
  },
  {
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('number')) {
    tags.add('number');
  }
  if (body.includes('infinite') || body.includes('infinity')) {
    tags.add('infinity');
  }
  if (body.includes('degree')) {
    tags.add('degree');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-quantum-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  traces.push({
    ...
    type: 'SUBLATES',
    reason:
      'Number contradiction transitions into extensive/intensive quantum.',
  });
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedQuantumTopicMapIR',
    'IntegratedQuantumTopicMapIR',
    'integratedQuantumTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Quantum section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
