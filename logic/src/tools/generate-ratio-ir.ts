...
import { inverseIR } from '@relative/being/quantity/ratio/inverse-ir';
import { powersIR } from '@relative/being/quantity/ratio/powers-ir';
import { ratioIR } from '@relative/being/quantity/ratio/ratio-ir';
import { INVERSE_TOPIC_MAP } from '@relative/being/quantity/ratio/sources/inverse-topic-map';
import { POWERS_TOPIC_MAP } from '@relative/being/quantity/ratio/sources/powers-topic-map';
import { RATIO_TOPIC_MAP } from '@relative/being/quantity/ratio/sources/ratio-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'ratio-ir.ts',
    exportName: 'ratioIR',
    statesExportName: 'ratioStates',
    ir: ratioIR,
  },
  {
    fileName: 'inverse-ir.ts',
    exportName: 'inverseIR',
    statesExportName: 'inverseStates',
    ir: inverseIR,
  },
  {
    fileName: 'powers-ir.ts',
    exportName: 'powersIR',
    statesExportName: 'powersStates',
    ir: powersIR,
  },
];
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('inverse')) {
    tags.add('inverse');
  }
  if (body.includes('power')) {
    tags.add('powers');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-ratio-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedRatioTopicMapIR',
    'IntegratedRatioTopicMapIR',
    'integratedRatioTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Ratio section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
