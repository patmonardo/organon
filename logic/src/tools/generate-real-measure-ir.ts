...
import { measurelessIR } from '@relative/being/measure/real-measure/measureless-ir';
import { measuresIR } from '@relative/being/measure/real-measure/measures-ir';
import { nodalLinesIR } from '@relative/being/measure/real-measure/nodal-lines-ir';
import { MEASURELESS_TOPIC_MAP } from '@relative/being/measure/real-measure/sources/measureless-topic-map';
import { MEASURES_TOPIC_MAP } from '@relative/being/measure/real-measure/sources/measures-topic-map';
import { NODAL_LINES_TOPIC_MAP } from '@relative/being/measure/real-measure/sources/nodal-lines-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'measures-ir.ts',
    exportName: 'measuresIR',
    statesExportName: 'measuresStates',
    ir: measuresIR,
  },
  {
    fileName: 'nodal-lines-ir.ts',
    exportName: 'nodalLinesIR',
    statesExportName: 'nodalLinesStates',
    ir: nodalLinesIR,
  },
  {
    fileName: 'measureless-ir.ts',
    exportName: 'measurelessIR',
    statesExportName: 'measurelessStates',
    ir: measurelessIR,
  },
];
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('affinity')) {
    tags.add('affinity');
  }
  if (body.includes('nodal')) {
    tags.add('nodal');
  }
  if (body.includes('leap')) {
    tags.add('leap');
  }
  if (body.includes('measureless') || body.includes('infinite')) {
    tags.add('measureless');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-real-measure-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  const sourceSpecs: SourceSpec[] = [
    {
      ...
      topicMap: MEASURES_TOPIC_MAP.entries,
    },
    {
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedRealMeasureTopicMapIR',
    'IntegratedRealMeasureTopicMapIR',
    'integratedRealMeasureTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Real-measure section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
