...
import { specificQuantityIR } from '@relative/being/measure/specific-quantity/specific-quantity-ir';
import { specificQuantumIR } from '@relative/being/measure/specific-quantity/specific-quantum-ir';
import { specifyingMeasureIR } from '@relative/being/measure/specific-quantity/specifying-measure-ir';
import { BEING_FOR_ITSELF_TOPIC_MAP } from '@relative/being/measure/specific-quantity/sources/being-for-itself-topic-map';
import { SPECIFIC_QUANTUM_TOPIC_MAP } from '@relative/being/measure/specific-quantity/sources/specific-quantum-topic-map';
import { SPECIFYING_MEASURE_TOPIC_MAP } from '@relative/being/measure/specific-quantity/sources/specifying-measure-topic-map';
...
const sectionDialecticSpecs: SectionDialecticSpec[] = [
  {
    fileName: 'specific-quantum-ir.ts',
    exportName: 'specificQuantumIR',
    statesExportName: 'specificQuantumStates',
    ir: specificQuantumIR,
  },
  {
    fileName: 'specifying-measure-ir.ts',
    exportName: 'specifyingMeasureIR',
    statesExportName: 'specifyingMeasureStates',
    ir: specifyingMeasureIR,
  },
  {
    fileName: 'specific-quantity-ir.ts',
    exportName: 'specificQuantityIR',
    statesExportName: 'specificQuantityStates',
    ir: specificQuantityIR,
  },
];
...
function deriveTags(chunk: IntegratedChunk): string[] {
  ...
  if (body.includes('quantum')) {
    tags.add('quantum');
  }
  if (body.includes('exponent')) {
    tags.add('exponent');
  }
  if (body.includes('ratio')) {
    tags.add('ratio');
  }
...
function buildCypher(ir: IntegratedIR): string {
  ...
  statements.push('// Generated by src/tools/generate-specific-quantity-ir.ts');
  statements.push('');
  statements.push(
...
async function main() {
  ...
  const sourceSpecs: SourceSpec[] = [
    {
      ...
      sourceFile:
        'relative/being/measure/specific-quantity/sources/specific-quantum.txt',
      topicMap: SPECIFIC_QUANTUM_TOPIC_MAP.entries,
    },
    {
      id: 'source-specifying-measure',
      title: 'B. SPECIFYING MEASURE',
      sourceFile:
        'relative/being/measure/specific-quantity/sources/specifying-measure.txt',
      topicMap: SPECIFYING_MEASURE_TOPIC_MAP.entries,
    },
    {
      id: 'source-being-for-itself-measure',
      title: 'C. THE BEING-FOR-ITSELF IN MEASURE',
      sourceFile:
        'relative/being/measure/specific-quantity/sources/being-for-itself.txt',
      topicMap: BEING_FOR_ITSELF_TOPIC_MAP.entries,
    },
  ];
  ...
  const topLevelTs = renderIntegratedTopicMapFile(
    'integratedSpecificQuantityTopicMapIR',
    'IntegratedSpecificQuantityTopicMapIR',
    'integratedSpecificQuantityTopicMap',
    ir,
  );
  ...
  const sectionDialecticOutputs = sectionDialecticSpecs.map((spec) => {
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join('\n');
      throw new Error(
        `Specific-quantity section DialecticIR validation failed (${spec.fileName}):\n${issues}`,
      );
    }
