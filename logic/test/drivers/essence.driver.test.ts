...
import { DefaultEssenceDriver, processShapes, commitShapes } from '../../src/absolute/essence/essence';
...
describe('EssenceDriver → ShapeEngine (ActiveShape)', () => {
  it('processes and commits ActiveShapes (create + update)', async () => {
    ...
    // process via driver
    const { actions, snapshot } = await DefaultEssenceDriver.processShapes(shapes, { repo });
    expect(snapshot.count).toBe(2);
    expect(actions.some(a => a.type === 'shape.upsert' && a.id === 'shape:one')).toBe(true);
  ...
  it('handles revoked (deletes)', async () => {
    // seed create
    await DefaultEssenceDriver.commitShapes([{ id: 'shape:x', name: 'X', active: true }], { repo });
    // revoke
    const { actions } = await processShapes([{ id: 'shape:x', revoked: true }], { repo });
    expect(actions.some(a => a.type === 'shape.delete' && a.id === 'shape:x')).toBe(true);
    const { commitResult } = await commitShapes([{ id: 'shape:x', revoked: true }], { repo });
  ...
  it('publishes events on the provided bus (created + core.set in one batch)', async () => {
    ...
    const events: string[] = [];
    bus.subscribe('shape.created', (e) => events.push(e.kind));
    bus.subscribe('shape.core.set', (e) => events.push(e.kind));
    ...
    const { commitResult } = await commitShapes(shapes, { repo: localRepo, bus });
    expect(commitResult.success).toBe(true);
    expect(events).toEqual(['shape.created', 'shape.core.set']);
  });
});
