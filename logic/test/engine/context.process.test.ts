...
describe("ContextEngine.process/commit (ADR-0003)", () => {
  it("upserts an active context and persists via repo", async () => {
    ...
    expect(kinds).toEqual(["context.created"]);
  ...
  it("deletes when revoked=true and id present", async () => {
    ...
    expect(created.kind).toBe("context.created");
    ...
    expect(kinds).toEqual(["context.deleted"]);
  ...
  it("is idempotent across repeated upsert commits (create then core.set)", async () => {
    ...
    expect((r1.events ?? []).map((e: any) => e.kind)).toEqual(["context.created"]);
    expect((r2.events ?? []).map((e: any) => e.kind)).toEqual(["context.core.set"]);
  });
