...
import { ListShape, ListItem } from '@/model/schema/list';
import { LinkRenderer } from '../button/link';

/**
 * ListRenderer - Client Component
 * Renders a ListShape directly to React elements
 */
export function ListRenderer({ list }: { list: ListShape }) {
  ...
  const filteredItems = list.items.filter((item: ListItem) => {
      ...
      {list.relations && (
        <div className="flex justify-end space-x-2">
          {list.relations.map((link, index) => (
            <span key={index}><LinkRenderer link={link} /></span>
          ))}
        </div>
      )}
